...
namespace rm {
    ...
    bool SerialPort::Send(Response &message) {
        ...
        sendBuffer[10] = rm::LookupCRC(sendBuffer, 10);

        return write(fd, sendBuffer, 11) > 0;
    }
    ...
    bool SerialPort::Receive(Request &message) {
        ...
        if (read(fd, receiveBuffer, 12) == 12) {
            if (receiveBuffer[0] != 0x38) {
                std::cout<<"038 err"<<std::endl;
                return false;
            }

            if (receiveBuffer[11] != LookupCRC(receiveBuffer, 11)) {
                std::cout<<"recive err"<<std::endl;
                return false;
            }
            ...
            message.yaw.bit[0] = receiveBuffer[7];
            message.yaw.bit[1] = receiveBuffer[8];
            message.yaw.bit[2] = receiveBuffer[9];
            message.yaw.bit[3] = receiveBuffer[10];
